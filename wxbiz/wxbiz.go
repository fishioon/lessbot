...
func (s *Wxbiz) VerifyURL(sign, ts, nonce, echostr string) (string, error) {
	return s.Decode(sign, ts, nonce, echostr)
}
...
func (s *Wxbiz) UnpackMsg(sign, ts, nonce string, data []byte) (*Msg, error) {
	...
	plain, err := s.Decode(sign, ts, nonce, string(msgRecv.Encrypt))
	if err != nil {
		return nil, err
	}
	m := new(Msg)
	if err = xml.Unmarshal([]byte(plain), m); err != nil {
		return nil, err
	}
	return m, nil
}
...
func (s *Wxbiz) PackMsg(msg *MsgReply, ts, nonce string) (*MsgSend, []byte, error) {
	...
	res, err := s.Encode(ts, nonce, string(data))
	if err != nil {
		return nil, nil, err
	}
...
func (s *Wxbiz) Decode(sign, ts, nonce, data string) (string, error) {
	...
	plainMsg, err := s.ParsePlainText(plaintext)
	if err != nil {
		return "", err
	}
	return plainMsg.Msg, nil
}
